...
app.controller('appCtrl', function($scope, $http) {
	...
	$scope.newHouseNameCheck = function () {
		...
		responsePromise = $http.get(iisnodeRoute + '/api/nameCheck', config)
		// promise.then(onFullfilled, onRejected)
		responsePromise.then(
			function (response) {
				console.log(response.status)
				$scope.nameCheckResponse.statusCode = response.status;
			...
			function (response) {
				console.log(response.status)
				$scope.loading = false
				// handle http 422 by providing message to user instead of rule results
				$scope.nameCheckResponse.statusCode = response.status;
				$scope.nameCheckResponse.message = response.data.error;
				$scope.nameCheckResponse.messageClass = 'label label-danger'
			}
		);
	}
